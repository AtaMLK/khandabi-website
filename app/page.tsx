...
const projects = [
  ["01", "PROJECT ARCHIVE", "https://images.unsplash.com/photo-1565610222536-ef125c59da2e?auto=format&fit=crop&w=1800&q=90"],
  ["02", "INSTALLATION STUDY", "https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&w=1800&q=90"],
  ["03", "LINE DEVELOPMENT", "https://images.unsplash.com/photo-1581094794329-c8112a89af12?auto=format&fit=crop&w=1800&q=90"],
];
...
export default function Home() {
  ...
  useGSAP(() => {
    ...
    const line = gsap.timeline({ scrollTrigger: { trigger: ".production", start: "top top", end: "+=2600", scrub: 1, pin: true } });
    ...
    const project = gsap.timeline({ scrollTrigger: { trigger: ".projects", start: "top top", end: "+=2400", scrub: 1, pin: true } });
    project.fromTo(".project-progress", { scaleX: 0, transformOrigin: "left" }, { scaleX: 1, duration: 1 });
    projects.slice(1).forEach((_, i) => {
      const n = i + 1;
      project.to(`.project-photo-${n - 1}`, { opacity: 0, scale: .94, duration: .45 }, "+=.35")
        .fromTo(`.project-photo-${n}`, { opacity: 0, scale: 1.08 }, { opacity: 1, scale: 1, duration: .65 }, "<")
        .fromTo(`.project-label-${n}`, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: .45 }, "<");
    });
  }, { scope: root });
  ...
  return <main ref={root} className="bg-[#f3f1ec] text-[#111820]">
    <nav className="fixed left-0 right-0 top-0 z-50 flex h-20 items-center justify-between px-6 text-white mix-blend-difference md:px-10">
      ...
      <div className="hidden items-center gap-8 text-[10px] font-semibold uppercase tracking-[.18em] md:flex"><a href="#products">Applications</a><a href="#machines">Machinery</a><a href="#production">Production Lines</a><a href="#engineering">Engineering</a><a href="#projects">Projects</a></div>
    ...
    <section id="top" className="hero relative h-[100svh] min-h-[720px] overflow-hidden bg-black text-white">
      <div className="hero-photo absolute inset-0"><Image src="https://images.unsplash.com/photo-1558961363-fa8fdf82db35?auto=format&fit=crop&w=2400&q=90" alt="Confectionery production" fill priority className="object-cover opacity-80" sizes="100vw" /><div className="absolute inset-0 bg-black/45" /></div><div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/15 to-black/30" />
    ...
    <section id="machines" className="machines relative h-screen overflow-hidden bg-[#0c1116] text-white"><div className="absolute inset-0">{machines.map(([no, title, type, image], i) => <div key={no} className={`machine-photo machine-photo-${i} absolute inset-0 ${i ? "opacity-0" : "opacity-100"}`}><Image src={image} alt={title} fill className="object-cover" sizes="100vw" /><div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/40 to-black/20" /></div>)}</div><div className="relative z-10 grid h-full grid-cols-1 gap-10 px-6 py-24 md:grid-cols-[.75fr_1.25fr] md:px-10"><div className="flex flex-col justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.25em] text-[#e76f32]">03 / Machinery</p><h2 className="mt-6 text-[clamp(3.5rem,7vw,7.5rem)] font-medium leading-[.82] tracking-[-.065em]">THE<br />MACHINES<br /><span className="text-[#e76f32]">AT WORK.</span></h2></div><p className="max-w-sm text-sm leading-7 text-white/55">A visual index of the equipment behind the production process.</p></div><div className="flex flex-col justify-end"><div className="mb-8 border-t border-white/25 pt-5 text-[10px] uppercase tracking-[.2em] text-white/50">Machine range / 06</div>{machines.map(([no, title, type], i) => <div key={no} className={`machine-row-${i} flex items-center gap-5 border-t border-white/15 py-3 ${i ? "opacity-0" : "opacity-100"}`}><span className="w-8 text-[10px] text-[#e76f32]">{no}</span><span className="flex-1 text-lg font-medium md:text-2xl">{title}</span><span className="hidden text-[9px] uppercase tracking-[.16em] text-white/40 md:block">{type}</span></div>)}</div></div></section>

    <section id="production" className="production relative h-screen overflow-hidden bg-black text-white"><div className="production-photo absolute inset-0"><Image src="https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&w=2400&q=90" alt="Production line" fill className="object-cover opacity-55" sizes="100vw" /></div><div className="absolute inset-0 bg-black/55" /><div className="relative z-10 flex h-full flex-col justify-between p-6 md:p-10"><div className="flex justify-between"><p className="text-[10px] font-bold uppercase tracking-[.25em] text-white/60">04 / Production lines</p><p className="text-[10px] uppercase tracking-[.2em] text-white/50">Scroll →</p></div><div><h2 className="max-w-5xl text-[clamp(3.5rem,8vw,8rem)] font-medium leading-[.82] tracking-[-.07em]">ONE FACTORY.<br /><span className="text-[#e76f32]">ONE FLOW.</span></h2><div className="mt-14 h-px w-full bg-white/30"><div className="process-track h-full w-full bg-white" /></div><div className="mt-6 grid grid-cols-2 gap-y-5 md:grid-cols-6">{process.map((step, i) => <div className="process-item text-[10px] font-bold uppercase tracking-[.18em] opacity-0" key={step}><span className="text-[#e76f32]">0{i + 1}</span><br />{step}</div>)}</div></div></div></section>
    ...
    <section id="projects" className="projects relative h-screen overflow-hidden bg-[#0b1015] text-white"><div className="absolute inset-0">{projects.map(([no, title, image], i) => <div key={no} className={`project-photo project-photo-${i} absolute inset-0 ${i ? "opacity-0" : "opacity-100"}`}><Image src={image} alt="Temporary project placeholder" fill className="object-cover" sizes="100vw" /><div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/30 to-black/60" /></div>)}</div><div className="relative z-10 flex h-full flex-col justify-between px-6 py-24 md:px-10 md:py-28"><div className="flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.25em] text-[#e76f32]">07 / Projects & installations</p><h2 className="mt-6 text-[clamp(3.5rem,7vw,8rem)] font-medium leading-[.82] tracking-[-.07em]">BUILT.<br />INSTALLED.<br /><span className="text-[#e76f32]">RUNNING.</span></h2></div><span className="hidden text-[10px] uppercase tracking-[.2em] text-white/45 md:block">Scroll to browse</span></div><div className="max-w-4xl"><div className="flex items-end justify-between gap-8"><div>{projects.map(([no, title], i) => <div key={no} className={`project-label-${i} ${i ? "opacity-0" : "opacity-100"}`}><p className="text-[10px] uppercase tracking-[.2em] text-[#e76f32]">{no} / {title}</p><p className="mt-3 max-w-xl text-sm leading-6 text-white/60">Temporary visual only. Verified Khandabi project data and catalogue imagery will replace this placeholder.</p></div>)}</div><span className="text-[10px] uppercase tracking-[.2em] text-white/45">Archive / 03</span></div><div className="mt-7 h-px w-full bg-white/20"><div className="project-progress h-full w-full origin-left bg-[#e76f32]" /></div></div></div></section>
