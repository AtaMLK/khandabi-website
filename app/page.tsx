...
import ProductionLinesSlider from "./components/ProductionLinesSlider";
...
export default function Home() {
  ...
  useGSAP(() => {
    ...
    gsap.timeline({ defaults: { ease: "power3.out" } }).from(".hero-kicker", { y: 24, opacity: 0, duration: .7 }).from(".hero-line", { yPercent: 110, duration: 1, stagger: .08 }, "-=.35").from(".hero-copy", { y: 24, opacity: 0, duration: .7 }, "-=.5");
    ...
    gsap.from(".lines-heading", { y: 45, opacity: 0, duration: .8, scrollTrigger: { trigger: ".lines", start: "top 75%" } });
  ...
  return <main ref={root} className="bg-[#f3f1ec] text-[#111820]">
    <nav className="fixed left-0 right-0 top-0 z-50 flex h-20 items-center justify-between px-6 text-white mix-blend-difference md:px-10"><Link href="#top" className="text-xl font-black tracking-[-.04em]">KHANDABI<span className="text-[#e76f32]">.</span></Link><div className="hidden items-center gap-7 text-[10px] font-semibold uppercase tracking-[.18em] md:flex"><Link href="/applications">Applications</Link><Link href="#machinery">Machinery</Link><Link href="#lines">Lines</Link><Link href="/customers">Customers</Link><Link href="/exhibitions">Exhibitions</Link><Link href="/about">About</Link><Link href="/contact">Contact</Link></div><div className="flex items-center gap-2"><a href="https://exhibitor-manual-004.s3.ap-south-1.amazonaws.com/Production/exb_doc/2016/21901/catalogue-te_966885.pdf" target="_blank" rel="noreferrer" className="hidden border border-white/50 px-4 py-2 text-[10px] font-bold uppercase tracking-[.16em] transition hover:bg-white hover:text-black lg:block">Catalogue ↗</a><Link href="/contact" className="border border-white/50 px-4 py-2 text-[10px] font-bold uppercase tracking-[.16em] transition hover:bg-white hover:text-black">Request a quote</Link></div></nav>

    <section id="top" className="hero relative h-[100svh] min-h-[680px] overflow-hidden bg-black text-white"><div className="hero-photo absolute inset-0"><Image src="https://images.unsplash.com/photo-1558961363-fa8fdf82db35?auto=format&fit=crop&w=2400&q=90" alt="Confectionery production" fill priority className="object-cover opacity-80" sizes="100vw" /><div className="absolute inset-0 bg-black/45" /></div><div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-black/30" /><div className="relative z-10 flex h-full flex-col justify-end px-6 pb-14 md:px-10 md:pb-20"><p className="hero-kicker mb-7 text-[10px] font-bold uppercase tracking-[.28em] text-white/70">KHANDABI / CONFECTIONERY MACHINERY</p><h1 className="max-w-[1200px] overflow-hidden text-[clamp(4rem,10vw,10rem)] font-semibold leading-[.82] tracking-[-.07em]"><span className="hero-line block">WE BUILD</span><span className="hero-line block pl-[8vw]">THE PROCESS.</span></h1><div className="hero-copy mt-8 flex max-w-4xl flex-col justify-between gap-8 md:flex-row md:items-end"><p className="max-w-lg text-base leading-7 text-white/75 md:text-lg">Machinery and complete production systems for chocolate, candy, bars, nougat, fondant, halva and more.</p><span className="text-[10px] uppercase tracking-[.2em] text-white/60">Scroll to explore ↓</span></div></div></section>
    ...
    <footer className="flex flex-col justify-between gap-6 bg-black px-6 py-9 text-white md:flex-row md:items-center md:px-10"><span className="text-xl font-black tracking-[-.04em]">KHANDABI<span className="text-[#e76f32]">.</span></span><div className="flex flex-wrap gap-6 text-[10px] uppercase tracking-[.18em] text-white/40"><Link href="/applications">Applications</Link><Link href="#machinery">Machinery</Link><Link href="#lines">Lines</Link><Link href="/customers">Customers</Link><Link href="/exhibitions">Exhibitions</Link><Link href="/about">About</Link><Link href="/contact">Contact</Link></div><span className="text-[10px] uppercase tracking-[.18em] text-white/40">© Khandabi Machinery Co.</span></footer>
  </main>;
}
