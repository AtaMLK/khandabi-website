...
export default function MachineryCatalogue({ machines }: { machines: Machine[] }) {
  ...
  return <>
    ...
    {active && <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/45 px-4 py-6 backdrop-blur-[3px]" role="dialog" aria-modal="true" aria-label={active.title} onClick={(event) => { if (event.target === event.currentTarget) setActive(null); }}><article className="relative grid max-h-[90vh] w-full max-w-[960px] overflow-auto bg-[#f3f1ec] text-[#111820] shadow-[0_30px_100px_rgba(0,0,0,.35)] md:grid-cols-[1.08fr_.92fr] md:overflow-hidden"><button type="button" onClick={() => setActive(null)} aria-label="Close preview" className="absolute right-5 top-4 z-20 flex h-9 w-9 items-center justify-center bg-black/70 text-xl text-white transition hover:bg-[#e76f32]">×</button><div className="relative min-h-[300px] overflow-hidden bg-[#151a20] md:min-h-[540px]"><img src={active.image} alt={active.title} className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/55 to-transparent" /></div><div className="flex min-h-[300px] flex-col justify-between p-8 md:min-h-[540px] md:p-12"><div><p className="text-[10px] font-bold uppercase tracking-[.22em] text-[#e76f32]">{active.category}</p><h3 className="mt-5 text-[clamp(2.5rem,4.5vw,4.8rem)] font-medium leading-[.86] tracking-[-.06em]">{active.title}</h3><div className="mt-7 h-px w-12 bg-[#e76f32]" /><p className="mt-6 max-w-md text-sm leading-7 text-black/60">{active.description}</p><div className="mt-8 grid grid-cols-2 gap-px border border-black/10 bg-black/10"><div className="bg-[#f3f1ec] p-4"><span className="text-[9px] uppercase tracking-[.16em] text-black/35">Capacity</span><p className="mt-2 text-sm">{active.capacity || "Not specified"}</p></div><div className="bg-[#f3f1ec] p-4"><span className="text-[9px] uppercase tracking-[.16em] text-black/35">Catalogue</span><p className="mt-2 text-sm">Page {active.cataloguePage ?? "—"}</p></div></div></div><Link href={`/machinery/${active.slug}`} className="mt-10 flex w-fit items-center gap-4 bg-[#e76f32] px-6 py-4 text-[10px] font-bold uppercase tracking-[.16em] text-white transition hover:bg-[#111820]">View full product <span>→</span></Link></div></article></div>}
  </>;
}
