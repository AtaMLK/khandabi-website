...
export default function MachineryCatalogue({ machines }: { machines: Machine[] }) {
  ...
  return <>
    <section className="mx-4 border border-black/10 bg-white md:mx-10">
      <div className={`relative overflow-hidden border-b border-black/10 bg-[#111820] text-white transition-[max-height,opacity] duration-300 ${preview ? "max-h-[430px] opacity-100" : "max-h-0 opacity-0"}`}>
        {preview && <div className="grid min-h-[300px] md:grid-cols-[40%_1fr]">
          <div className="relative min-h-[250px] overflow-hidden bg-[#151a20]"><img src={preview.image} alt={preview.title} className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"/></div>
          <div className="flex flex-col justify-between p-7 md:p-8"><div><p className="text-[9px] font-bold uppercase tracking-[.22em] text-[#e76f32]">{preview.category}</p><h3 className="mt-3 text-[clamp(2rem,4vw,4rem)] font-medium leading-[.86] tracking-[-.06em]">{preview.title}</h3><p className="mt-5 max-w-3xl text-sm leading-6 text-white/60">{preview.description}</p><div className="mt-5 flex flex-wrap gap-2"><span className="border border-white/10 px-3 py-2 text-[9px] uppercase tracking-[.15em] text-white/55">Capacity: {preview.technical.capacity || "Not specified"}</span><span className="border border-white/10 px-3 py-2 text-[9px] uppercase tracking-[.15em] text-white/55">Catalogue: {preview.cataloguePages.length ? preview.cataloguePages.join(" / ") : "—"}</span></div></div><div className="flex flex-wrap gap-3 pt-6"><button type="button" onClick={()=>setSelected(preview)} className="bg-[#e76f32] px-5 py-3 text-[9px] font-bold uppercase tracking-[.18em] transition hover:bg-white hover:text-black">Quick view</button><Link href={`/machinery/${preview.slug}`} className="border border-white/20 px-5 py-3 text-[9px] font-bold uppercase tracking-[.18em] transition hover:bg-white hover:text-black">Full product →</Link></div></div>
        </div>}
      </div>
