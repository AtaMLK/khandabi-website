...
export default function MachineryCatalogue({ machines }: { machines: Machine[] }) {
  ...
  const openHover = (machine: Machine, index: number) => {
  ...
  const openSelected = (machine: Machine) => {
    if (closeTimer.current) clearTimeout(closeTimer.current);
    setHovered(null);
    setSelected(machine);
  };
      ...
      <section className="mx-6 border border-black/10 md:mx-10">
        <div className="grid grid-cols-1 md:grid-cols-2">
          {machines.map((machine, i) => {
            ...
            const isHovered = hovered?.machine.slug === machine.slug;
            ...
            return (
              <div
                ...
                onMouseEnter={() => openHover(machine, i)}
                onMouseLeave={closeHover}
                onFocus={() => openHover(machine, i)}
                onBlur={closeHover}
                onClick={() => openSelected(machine)}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    openSelected(machine);
                  }
                }}
                tabIndex={0}
                role="button"
              ...
              >
                ...
                {/* Desktop hover preview is placed on the opposite half of the screen. */}
                {isHovered && (
                  <div
                    className={`pointer-events-none fixed inset-y-0 z-[9000] hidden w-1/2 items-center px-5 md:flex md:px-8 ${
                    ...
                    <div className="grid w-full max-w-[720px] overflow-hidden bg-[#f3f1ec] text-[#111820] shadow-[0_30px_100px_rgba(0,0,0,.28)] md:grid-cols-[1.05fr_.95fr]">
                      <div className="relative min-h-[420px] overflow-hidden bg-[#151a20]">
                        ...
                        <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4 text-white">
                          ...
                          {machine.cataloguePages.length > 0 && (
                            <span className="text-[9px] uppercase tracking-[.15em] text-white/65">Catalogue {machine.cataloguePages.join(" / ")}</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex min-h-[420px] flex-col justify-between overflow-hidden p-7 md:p-9">
                        <div className="overflow-y-auto pr-1">
                          ...
                          <h3 className="mt-4 text-[clamp(2rem,3.6vw,4rem)] font-medium leading-[.86] tracking-[-.06em]">{machine.title}</h3>
                        ...
                        <span className="mt-6 shrink-0 text-[9px] font-bold uppercase tracking-[.18em] text-black/35">Click to open product</span>
                      </div>
                    </div>
                  </div>
                )}
      ...
      {selected && (
        ...
        >
          <article className="relative grid max-h-[90vh] w-full max-w-[1100px] overflow-auto bg-[#f3f1ec] text-[#111820] shadow-[0_30px_100px_rgba(0,0,0,.35)] md:grid-cols-[1.05fr_.95fr] md:overflow-hidden">
            <button type="button" onClick={() => setSelected(null)} aria-label="Close preview" className="absolute right-5 top-4 z-20 flex h-9 w-9 items-center justify-center bg-black/70 text-xl text-white transition hover:bg-[#e76f32]">×</button>
            ...
            <div className="relative min-h-[320px] overflow-hidden bg-[#151a20] md:min-h-[600px]">
              ...
              <div className="absolute bottom-6 left-6 right-6 flex flex-wrap items-center justify-between gap-3 text-[9px] font-bold uppercase tracking-[.2em] text-white/70">
                ...
                {selected.cataloguePages.length > 0 && <span>Catalogue {selected.cataloguePages.join(" / ")}</span>}
              </div>
            </div>

            <div className="flex min-h-[320px] flex-col justify-between overflow-y-auto p-8 md:min-h-[600px] md:p-11">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[.22em] text-[#e76f32]">{selected.category}</p>
                <h3 className="mt-5 text-[clamp(2.5rem,4.5vw,4.8rem)] font-medium leading-[.86] tracking-[-.06em]">{selected.title}</h3>
                <div className="mt-7 h-px w-12 bg-[#e76f32]" />
                <p className="mt-6 max-w-md text-sm leading-7 text-black/60">{selected.description}</p>

                <div className="mt-8 grid grid-cols-2 gap-px border border-black/10 bg-black/10">
                  <div className="bg-[#f3f1ec] p-4"><span className="text-[9px] uppercase tracking-[.16em] text-black/35">Capacity</span><p className="mt-2 text-sm">{selected.technical.capacity || "Not specified"}</p></div>
                  <div className="bg-[#f3f1ec] p-4"><span className="text-[9px] uppercase tracking-[.16em] text-black/35">Catalogue</span><p className="mt-2 text-sm">{selected.cataloguePage ? `Page ${selected.cataloguePage}` : "Not specified"}</p></div>
                </div>

                {selected.catalogueImages.length > 0 && (
                  <div className="mt-8">
                    <p className="text-[9px] font-bold uppercase tracking-[.18em] text-black/35">Catalogue references</p>
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      {selected.catalogueImages.slice(0, 6).map((src, index) => (
                        <img key={`${src}-${index}`} src={src} alt={`${selected.title} catalogue reference ${index + 1}`} className="aspect-[4/3] w-full object-cover bg-black/5" />
                      ))}
                    </div>
                  </div>
                )}
              ...
              <Link href={`/machinery/${selected.slug}`} className="mt-10 flex w-fit items-center gap-4 bg-[#e76f32] px-6 py-4 text-[10px] font-bold uppercase tracking-[.16em] text-white transition hover:bg-[#111820]">View full product <span>→</span></Link>
            </div>
          </article>
        </div>
      )}
