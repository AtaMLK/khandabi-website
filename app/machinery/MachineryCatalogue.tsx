...
export default function MachineryCatalogue({ machines }: { machines: Machine[] }) {
  ...
  const previewMachine = (machine: Machine) => setHovered(machine);
    ...
    <section className="mx-4 border border-black/10 bg-white md:mx-10">
      <div className="grid items-start lg:grid-cols-[minmax(0,1fr)_390px]">
        <div className="min-w-0">
          {groups.map((group, groupIndex) => <section key={group.name} className="border-b border-black/10 last:border-b-0">
            <header className="flex items-end justify-between gap-6 bg-[#f3f1ec] px-5 py-5 md:px-7"><div><p className="text-[9px] font-bold uppercase tracking-[.24em] text-[#e76f32]">{String(groupIndex + 1).padStart(2, "0")} / Product group</p><h3 className="mt-2 text-2xl font-medium tracking-[-.04em] md:text-3xl">{group.name}</h3></div><span className="text-[9px] uppercase tracking-[.18em] text-black/35">{group.machines.length} products</span></header>
            <div className="grid grid-cols-1 md:grid-cols-2">
              {group.machines.map((machine, index) => { const active = hovered?.slug === machine.slug; return <Link key={machine.slug} href={`/machinery/${machine.slug}`} onMouseEnter={() => previewMachine(machine)} onMouseOver={() => previewMachine(machine)} onPointerEnter={() => previewMachine(machine)} onPointerMove={() => previewMachine(machine)} onFocus={() => previewMachine(machine)} onClick={(event) => { event.preventDefault(); setHovered(machine); setSelected(machine); }} className={`group flex min-h-[82px] w-full items-center border-t border-black/10 px-5 text-left transition md:min-h-[92px] md:px-7 ${active ? "bg-[#e7e3da]" : "hover:bg-[#f5f2ec]"}`}><span className="w-8 shrink-0 text-[9px] tabular-nums text-[#e76f32]">{String(index + 1).padStart(2, "0")}</span><span className="flex-1 px-3 text-[12px] font-medium leading-5 md:text-[13px]">{machine.title}</span><span className="text-[#e76f32] transition-transform group-hover:translate-x-1">↗</span></Link>; })}
            </div>
          </section>)}
        </div>
        <aside className="hidden self-start border-l border-black/10 bg-[#111820] text-white lg:block">
          <div className="sticky top-20 min-h-[620px]">
            {hovered && <div className="flex min-h-[620px] flex-col">
              <div className="relative h-[280px] overflow-hidden bg-[#151a20]"><img key={hovered.slug} src={hovered.image} alt={hovered.title} className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"/><div className="absolute bottom-4 left-5 text-[8px] uppercase tracking-[.2em] text-white/60">KHANDABI / MACHINERY</div></div>
              <div className="flex flex-1 flex-col justify-between p-6"><div><p className="text-[9px] font-bold uppercase tracking-[.22em] text-[#e76f32]">{hovered.category}</p><h3 className="mt-3 text-4xl font-medium leading-[.86] tracking-[-.06em]">{hovered.title}</h3><div className="mt-5 h-px w-10 bg-[#e76f32]"/><p className="mt-4 text-xs leading-6 text-white/60">{hovered.description}</p><div className="mt-5 grid grid-cols-2 border border-white/10"><div className="border-r border-white/10 p-3"><span className="text-[8px] uppercase tracking-widest text-white/35">Capacity</span><p className="mt-2 text-[11px]">{hovered.technical.capacity || "Not specified"}</p></div><div className="p-3"><span className="text-[8px] uppercase tracking-widest text-white/35">Catalogue</span><p className="mt-2 text-[11px]">{hovered.cataloguePages.length ? hovered.cataloguePages.join(" / ") : "—"}</p></div></div></div><div className="flex gap-2 pt-6"><button type="button" onClick={() => setSelected(hovered)} className="bg-[#e76f32] px-4 py-3 text-[8px] font-bold uppercase tracking-[.17em] transition hover:bg-white hover:text-black">Quick view</button><Link href={`/machinery/${hovered.slug}`} className="border border-white/20 px-4 py-3 text-[8px] font-bold uppercase tracking-[.17em] transition hover:bg-white hover:text-black">Full product →</Link></div></div>
            </div>}
          </div>
        </aside>
      </div>
