...
export default function MachineryCatalogue({ machines }: { machines: Machine[] }) {
  ...
  return (
    <>
      <section className="mx-6 border border-black/10 md:mx-10">
        <div className="grid grid-cols-1 md:grid-cols-2">
          {machines.map((machine, i) => (
            <div
              key={machine.slug}
              className="group relative min-h-[92px] cursor-pointer border-b border-black/10 bg-transparent transition-colors duration-200 hover:bg-[#ebe7df] md:min-h-[102px]"
            ...
            >
              <div className="flex h-full min-h-[92px] w-full items-center px-5 py-5 md:min-h-[102px] md:px-8">
              ...
              {/* CSS hover preview: this does not depend on React state or mouse-leave timing. */}
              <div className="pointer-events-none fixed inset-0 z-[9000] flex items-center justify-center px-4 opacity-0 invisible transition-all duration-200 group-hover:visible group-hover:opacity-100 md:px-8">
                <div className="grid w-full max-w-[960px] overflow-hidden bg-[#f3f1ec] text-[#111820] shadow-[0_30px_100px_rgba(0,0,0,.28)] md:grid-cols-[1.08fr_.92fr]">
                  <div className="relative min-h-[280px] overflow-hidden bg-[#151a20] md:min-h-[500px]">
                  ...
                  <div className="flex min-h-[280px] flex-col justify-between p-7 md:min-h-[500px] md:p-11">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[.22em] text-[#e76f32]">{machine.category}</p>
                      <h3 className="mt-5 text-[clamp(2.2rem,4.5vw,4.8rem)] font-medium leading-[.86] tracking-[-.06em]">{machine.title}</h3>
                      <div className="mt-7 h-px w-12 bg-[#e76f32]" />
                      <p className="mt-6 max-w-md text-sm leading-7 text-black/60">{machine.description}</p>
                    </div>
                    <span className="mt-8 text-[9px] font-bold uppercase tracking-[.18em] text-black/35">Click to open product</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* Click/tap modal. Hover uses the CSS preview above; click gives the full interactive modal. */}
      {active && (
        <div
          className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/45 px-4 py-6 backdrop-blur-[3px]"
          onClick={(e) => { if (e.target === e.currentTarget) setActive(null); }}
        >
          <article className="relative grid max-h-[90vh] w-full max-w-[960px] overflow-auto bg-[#f3f1ec] text-[#111820] shadow-[0_30px_100px_rgba(0,0,0,.35)] md:grid-cols-[1.08fr_.92fr] md:overflow-hidden">
            <button type="button" onClick={() => setActive(null)} aria-label="Close preview" className="absolute right-5 top-4 z-20 flex h-9 w-9 items-center justify-center bg-black/70 text-xl text-white">×</button>
